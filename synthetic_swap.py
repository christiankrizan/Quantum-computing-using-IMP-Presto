...
def iswap_then_cz20_no_virtual_z_added_state_probability(
    ip_address,
    ext_clk_present,
    ...
    ''' Run an iSWAP gate, followed by a CZ20 gate.
        The prepared state is |10>.
        Note! No virtual-Z gates are ever added.
        
        The output is a state discriminated value, the target is |01>
    '''
    ...
    with pulsed.Pulsed(
        ...
        high_gate_f  = max( [coupler_ac_freq_iswap, coupler_ac_freq_cz20] )
        low_gate_f   = min( [coupler_ac_freq_iswap, coupler_ac_freq_cz20] )
        coupler_ac_freq_nco = high_gate_f - (high_gate_f - low_gate_f)/2 -250e6
        pls.hardware.configure_mixer(
        ...
        coupler_ac_freq_if_iswap = np.abs(coupler_ac_freq_nco - coupler_ac_freq_iswap)
        pls.setup_freq_lut(
            output_ports    = coupler_ac_port,
            group           = 0,
            frequencies     = coupler_ac_freq_if_iswap,
            phases          = 0.0,
        ...
        coupler_ac_freq_if_cz20 = np.abs(coupler_ac_freq_nco - coupler_ac_freq_cz20)
        pls.setup_freq_lut(
            output_ports    = coupler_ac_port,
            group           = 1,
            frequencies     = coupler_ac_freq_if_cz20,
            phases          = 0.0,
            phases_q        = -np.pi/2, # USB!
        )
