...
def iswap_sweep_duration_and_detuning(
    ...
    with pulsed.Pulsed(
        ...
        # Let's use the USB considering the NCO of the coupler AC port mixer.
        ...
        pls.setup_freq_lut(
            ...
            phases_q        = np.full_like(coupler_ac_freq_iswap_if_arr, -np.pi/2), # USB!
        )
...
def iswap_cross_ramsey(
    ...
    ''' Perform a cross-Ramsey experiment between two qubits,
        ...
        If there are Ramsey fringes in the readout data, then this
        experiment indicates phase coherence, the "i" in the iSWAP.
    '''
    ...
    with pulsed.Pulsed(
        ...
        # Coupler port mixer
        #coupler_ac_freq_iswap_nco = 200.13e6
        pls.hardware.configure_mixer(
            freq      = coupler_ac_freq_iswap_nco, # Fixed value for this sweep.
            out_ports = coupler_ac_port,
            tune      = True,
            sync      = (coupler_dc_port == []),
        )
        ...
        for ii in range(num_phases):
            ...
            pls.output_pulse(T, [control_pulse_pi_01_half_Vz_A, control_pulse_pi_01_half_Vz_B])
            T += control_duration_01
...
def tune_local_qubit_phases_of_iswap(
    ...
    elif phase_sweep_rad_min < -(2*np.pi):
        phase_sweep_rad_min = -(2*np.pi)
        
    if phase_sweep_rad_max > (2*np.pi):
        phase_sweep_rad_max = (2*np.pi)
    elif phase_sweep_rad_max < -(2*np.pi):
        phase_sweep_rad_max = -(2*np.pi)
