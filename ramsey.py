...
from data_exporter import \
    ...
    save
from noise import generate_noise_template
...
def ramsey01_with_noise_ro0(
    ...
    ## Assert legal noise type.
    noise_type = noise_type.lower()
    if not ((noise_type == "gaussian") or (noise_type == "white") or (noise_type == "pink")):
        raise ValueError("Error! Legal values for argument 'noise_type' is 'gaussian', 'white', or 'pink'.")
    ...
    with pulsed.Pulsed(
        ...
        repetition_rate = int(round(repetition_rate / plo_clk_T)) * plo_clk_T
        control_duration_01 = int(round(control_duration_01 / plo_clk_T)) * plo_clk_T
        noise_duration = int(round(noise_duration / plo_clk_T)) * plo_clk_T
        settling_time_of_bias_tee = int(round(settling_time_of_bias_tee / plo_clk_T)) * plo_clk_T
        ...
        # Noise port amplitude
        pls.setup_scale_lut(
            output_ports    = noise_port,
            group           = 0,
            scales          = noise_amp,
        )
        ...
        noise_pulse = pls.setup_template(
            output_port = noise_port,
            group       = 0,
            template    = noise_template,
            template_q  = noise_template,
            envelope    = True,
        )
        # Setup noise_pulse carrier tone, considering that there is a digital mixer.
        noise_freq_if = noise_nco - noise_center_freq
        pls.setup_freq_lut(
            output_ports = noise_port,
            group        = 0,
            frequencies  = np.abs(noise_freq_if),
            phases       = 0.0,
            phases_q     = bandsign(noise_freq_if),
        )
        ...
        for ii in range(len(delay_arr)):
            ...
            # Apply the frequency-swept pi_01 pulses,
            # and, also the noise pulse.
            pls.reset_phase(T, control_port)
            pls.reset_phase(T, noise_port)
            pls.output_pulse(T, control_pulse_pi_01_half)
            pls.output_pulse(T, noise_port)
            T += control_duration_01
            ...
            # Is the noise still playing?
            if not (T >= (T_begin + noise_duration)):
                T = (T_begin + noise_duration)
    ...
    if not pls.dry_run:
        ...
        string_arr_to_return.append(save(
            ...
            source_code_of_executing_file = '', #get_sourcecode(__file__),
            default_exported_log_file_name = default_exported_log_file_name,
            append_to_log_name_before_timestamp = '01_noise_added' + with_or_without_bias_string,
            append_to_log_name_after_timestamp  = '',
            select_resonator_for_single_log_export = '',
...
def ramsey01_echo_with_noise_ro0(
    ip_address,
    ext_clk_present,
    ...
    if not pls.dry_run:
        ...
        string_arr_to_return.append(save(
            ...
            source_code_of_executing_file = '', #get_sourcecode(__file__),
            default_exported_log_file_name = default_exported_log_file_name,
            append_to_log_name_before_timestamp = '01_echo_noise_added' + with_or_without_bias_string,
            append_to_log_name_after_timestamp  = '',
            select_resonator_for_single_log_export = '',
